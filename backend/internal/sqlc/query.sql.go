...
func (q *Queries) GetUsers(ctx context.Context, dollar_1 []string) ([]GetUsersRow, error) {
	...
	for rows.Next() {
		var i GetUsersRow
		if err := rows.Scan(&i.UserID, &i.School, &i.VerifiedAt); err != nil {
			return nil, err
		}
...
const upsertSchool = `-- name: UpsertSchool :one
...
func (q *Queries) UpsertSchool(ctx context.Context, arg UpsertSchoolParams) (Email, error) {
	row := q.db.QueryRow(ctx, upsertSchool, arg.UserID, arg.School)
	var i Email
	err := row.Scan(
		&i.UserID,
		&i.Email,
